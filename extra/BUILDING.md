...
# Additional setup for OCaml development on Windows

In order to use the [VSCode OCaml extension](https://marketplace.visualstudio.com/items?itemName=hackwaly.ocaml),

 * Add these to your PATH:
    - cygwin tools: `path\to\cygwin\bin`
    - runtime dlls: `path\to\cygwin\usr\x86_64-w64-mingw32\sys-root\mingw\bin`
    - ocaml bin: the output of `cygpath -w "$(which ocaml)"`
    - flexlink bin: `path\to\cygwin\usr\local\bin`
  - Add new env variables (actual paths may differ depending on your install path, username, and OCaml version):
    - `OCAML_TOPLEVEL_PATH=path\to\cygwin\home\*\.opam\*\lib\toplevel`
    - `OCAMLLIB=path\to\cygwin\home\*\.opam\*\lib\ocaml`
